            ...
            using (var sqlConnection = new MySqlConnection(_connectionDB))
            {
                ...
                using (MySqlTransaction trans = sqlConnection.BeginTransaction())
                {
                    ...
                    if (numberOfAffetedRows > 0)
                    {
                        ...
                        int insertNotice = this.InsertNotice(record.PrescriptionID, notificationEntities, sqlConnection, trans: trans);
                        if (resultDetail > 0 && insertNotice>0)
                        {
                            trans.Commit();
                            return record;
                        }
            ...
            using (var sqlConnection = new MySqlConnection(_connectionDB))
            {
                ...
                using (MySqlTransaction trans = sqlConnection.BeginTransaction())
                {
                    ...
                    if (numberOfAffectedRows > 0)
                    {
                        ...
                        int deleteNotice = DeleteNoticeByPres(record.PrescriptionID);
                        if (deleteRecord > 0 && deleteNotice>0)
                        {
                            int insertMedication = this.InsertMedication(record.PrescriptionID, medicationsEntity, sqlConnection, trans: trans);
                            int insertNotice = this.InsertNotice(record.PrescriptionID, notificationEntities, sqlConnection, trans: trans);
                            if (insertMedication > 0 && insertNotice>0)
                            {
                                trans.Commit();
                                return record;
                            }
